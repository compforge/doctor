...
import { createServiceCatalog, requireCaseRunnerCreateExtension, caseRunnerOutput,
  type CaseRunnerCreateExtension, type ServiceDefinition } from "../src";
...
test("Case adaptation preserves metadata and never constructs a runner during discovery", async () => {
  ...
  const service = { ...base, capabilities: { case: { endpoint: extension.endpoint, access: {}, caseSets: [cases], createRunner } } };
