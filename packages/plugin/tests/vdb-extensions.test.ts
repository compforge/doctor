...
test("VDB adapters remain offline and distinguish each data source", async () => {
  ...
  const declared = {
    ...
    extensions: [{ ...extension, run: withSummary({ title: "Target", fields: [] }, inspectTarget) }, { ...extension, id: "business-target", dataSource: "business", run: withSummary({ title: "Target", fields: [] }, inspectTarget) }]
  };
  ...
  expect((await found[0]!.run({} as never, undefined)).data).toBe(target);
