...
import { createServiceCatalog, requireVdbTargetInspectExtension, vdbTargetOutput,
  type ServiceDefinition, type VdbTargetInspectExtension } from "../src";
const target = { backend: "opensearch", store: "trace", endpoint: "http://search:9200", configurationKind: "plugin" };
const service: ServiceDefinition = { name: "search", component: { name: "test", repository: { forge: { name: "test" }, path: "test" } }, workloads: [], capabilities: {} };
...
test("VDB adapters remain offline and distinguish each data source", async () => {
  ...
  const trace = { id: "trace", kind: "vdb", backend: "opensearch", inspectTarget } as const;
  const declared = { ...service, capabilities: { dataSources: [trace, { ...trace, id: "business" }] } };
