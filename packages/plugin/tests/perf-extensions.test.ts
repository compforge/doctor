...
import { createServiceCatalog, perfScenariosOutput, requirePerfScenariosExtension,
  type ServiceDefinition, type ServicePerfScenario } from "../src";
const scenario: ServicePerfScenario = { id: "chat", title: "Chat", description: "Chat load", caseSetId: "chat",
...
test("Perf declarations adapt without executing a provider during discovery", async () => {
  const legacy = createServiceCatalog([{ ...service, capabilities: { perf: { scenarios: [scenario] } } }]);
  ...
  expect(() => createServiceCatalog([{ ...service, capabilities: { perf: { scenarios: [scenario] } }, extensions: [extension] }])).toThrow("not both");
});
