...
test("legacy Metric is adapted as a zero-access configuration function", async () => {
  const services = createServiceCatalog([{ ...base, capabilities: { metric } }]);
  ...
  expect(() => createServiceCatalog([{ ...base, capabilities: { metric }, extensions: [extension] }])).toThrow("not both");
});
