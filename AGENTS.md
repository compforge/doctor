...
本仓包含通用 CLI、Plugin SDK、产品中立的示例 Plugin，以及未来 Doctor server 的目录占位；不包含
...
| `plugins/example/` | 只演示协议接入的产品中立 Plugin |

更细的 CLI 分层与诊断领域索引见 `cli/AGENTS.md`。

## 关键约定

1. **业务与通用能力分离**：业务 Service 名称、拓扑、查询和判读规则只存在于具体 Plugin；CLI、SDK
   和 example Plugin 不依赖真实产品实现。
