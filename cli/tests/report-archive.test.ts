...
import { REPORT_ARCHIVE_SCRIPT, ReportArchive } from "../src/report/archive";

test("offline reader inflates the index and requested leaf only", () => {
  const writer = new ReportArchive();
  const first = writer.addPage("<p>中文证据</p>");
  const second = writer.addPage("<p>unopened evidence</p>");
  const embedded = writer.embedIndex({ version: 2, pages: [{ key: "first", label: "first", status: "delivered", ...first },
  ...
  const result = runInNewContext(REPORT_ARCHIVE_SCRIPT + `
    ...
    const text=fflate.strFromU8(readReportEntry(reportIndex.pages[0].entry));
