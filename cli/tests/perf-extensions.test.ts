...
const service: ServiceDefinition = { name: "app", aliases: ["chat"], component: { name: "test", repository: { forge: { name: "test" }, path: "test" } }, workloads: [], capabilities: {
  case: { endpoint: { host: "app", port: 8080 }, access: {},
...
const providerFor = (item = extension) => selectPerfProvider(createServiceCatalog([{ ...service, extensions: [item] }]), "chat");
