...
test("从业务镜像推断同 registry/namespace 的 doctor-debug", () => {
  ...
  expect(DOCTOR_DEBUG_IMAGE_VERSION).toBe("0.2.10");
...
`);
  ...
  expect(await resolveDebugImage(target, { config: "/does/not/exist" }, { interactive: false })).toEqual({
    image: "registry.example.com/dev/doctor-debug:0.2.10",
    source: "inferred",
  });
});
...
test("doctor image 无集群级 list pods 权限时提示手动输入", async () => {
  ...
  try {
    ...
    expect(write).toHaveBeenCalledWith(expect.stringContaining(
      "当前 Kubernetes 凭据没有集群级 list pods 权限，无法从 Pod 镜像自动发现 registry 和镜像 namespace；改为手动输入。",
    ));
  } finally {
    write.mockRestore();
  }
