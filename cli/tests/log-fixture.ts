...
export const logService = (name = "api"): ServiceDefinition => ({
  name,
  component: { name, repository: { forge: { name: "test" }, path: "fixtures/log" } },
...
export function logTarget(pod: string, hasPrevious = false, namespace = "test"): LogWorkloadTarget {
  return {
    instance: {
      platform: "kubernetes", environment: "default", workload: "main",
      namespace, pod, uid: pod + "-uid", container: "app"
    }, hasPrevious,
...
export const logIdentityExecutor: Executor = {
  run: async args => ({
    ok: true, exitCode: 0, stderr: "", durationMs: 1, timedOut: false, command: args,
    stdout: JSON.stringify({
      metadata: { name: args[2], uid: args[2] + "-uid" }, status: {
  ...
  return {
    run: async args => {
      ...
      const items = pods.map(pod => ({
        ...
        status: {
          phase: pod.phase, containerStatuses: pod.containers.map(container => ({
