  ...
  return {
    run: async args => {
      ...
      return { ok: true, exitCode: 0, stderr: "", durationMs: 1, timedOut: false, command: args,
        stdout: JSON.stringify({ items: pods.map(pod => ({
          ...
          status: { phase: pod.phase, containerStatuses: pod.containers.map(container => ({
            name: container.name, containerID: container.containerId, restartCount: container.restartCount,
            lastState: container.hasPreviousTerminated ? { terminated: { containerID: container.lastTermination?.containerId } } : {},
          })) },
        })) }),
      };
    },
