...
describe("Plugin archive lifecycle", () => {
  it("builds a deterministic self-contained archive and loads the installed definition", async () => {
    ...
    expect(result).toMatchObject({ ref: "example@0.0.6", installed: true });
    ...
    expect(plugin.version).toBe("0.0.6");
  ...
  it("rejects installed payload changes before importing Plugin code", async () => {
    const root = temporaryRoot();
    const archive = await buildPluginArchive(
      resolve(import.meta.dir, "../../plugins/example"),
      join(root, "dist"),
    );
    const result = await installPlugin(archive, join(root, "plugins"));
    appendFileSync(join(result.path, "plugin.mjs"), "\n// changed after install\n");

    await expect(loadInstalledPlugin(result.ref, join(root, "plugins"))).rejects.toThrow(
      "Plugin artifact content does not match",
    );
  });

  it("rejects installed manifest changes against the Host install receipt", async () => {
    const root = temporaryRoot();
    const archive = await buildPluginArchive(
      resolve(import.meta.dir, "../../plugins/example"),
      join(root, "dist"),
    ...
    const result = await installPlugin(archive, join(root, "plugins"));
    appendFileSync(join(result.path, "plugin.json"), "\n");

    await expect(loadInstalledPlugin(result.ref, join(root, "plugins"))).rejects.toThrow(
      "Plugin manifest does not match its install receipt",
    );
  });
});
