...
test("nested aggregates retain direct edges, reuse the same result and remain portable", async () => {
  ...
  expect(parents.map(parent => parent.manifest.command)).toEqual(["collect", "collect"]);
  const childIds = parents.flatMap(parent => parent.manifest.children!.map(child => child.executionId));
  ...
  for (const parent of parents) for (const child of parent.manifest.children!) {
    ...
    const saved = read(path) as CommandManifest;
...
test("diagnosis body is externalized and render rehydrates it from relocated evidence", async () => {
  ...
  bundle.writeManifest({ doctorVersion: "test", target: {}, inspectionFacts: facts, params: {}, startedAt: "start", finishedAt: "end" });
...
test("Data serializes multiple input selections without copying Facts into diagnoses", async () => {
  ...
  new EvidenceBundle(source).writeManifest({ doctorVersion: "test", target: {}, inspectionFacts: facts, params: {}, startedAt: "start", finishedAt: "end" });
