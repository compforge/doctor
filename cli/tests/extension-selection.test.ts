...
test("provider selection preserves Service and Extension identity without executing candidates", async () => {
  ...
  const first = { id: "first", kind: "tenant.list", access: {}, endpoint: { host: "one", port: 80 }, run };
...
test("directory opens only the selected operation and never invents user search", async () => {
  ...
  const catalog = createServiceCatalog([{
    ...service("directory"), extensions: [
      { id: "a", kind: "tenant.list", endpoint: { host: "directory", port: 80 }, access: {}, run: unused },
      { id: "b", kind: "tenant.list", endpoint: { host: "directory", port: 80 }, access: {}, run: selected },
      { id: "resolve", kind: "tenant.resolve", endpoint: { host: "directory", port: 80 }, access: {}, run: unused },
    ],
  }]);
  const opened: string[] = [];
