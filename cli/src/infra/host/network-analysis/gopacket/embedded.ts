...
declare global {
  ...
  var __DOCTOR_PCAP_ASSET__: string | undefined;
}
...
function readSeaAsset(): Uint8Array | undefined {
  ...
  return sea.isSea() ? new Uint8Array(sea.getRawAsset("doctor-pcap")) : undefined;
}

function readBunAsset(): Uint8Array | undefined {
  const path = globalThis.__DOCTOR_PCAP_ASSET__;
  return path ? readFileSync(path) : undefined;
}

/** Embedded helpers must be materialized before the operating system can execute them. */
export function resolveEmbeddedGopacketCommand(): string | undefined {
  if (extractedCommand) return extractedCommand;
  const bytes = readSeaAsset() ?? readBunAsset();
  if (!bytes) return undefined;

  const directory = mkdtempSync(join(tmpdir(), "doctor-pcap-"));
  const command = join(directory, "doctor-pcap");
