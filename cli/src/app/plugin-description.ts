...
export function formatServiceDescription(service: ServiceDescription): string {
  ...
  const { inspect, workloads, dependencies, stores, access } = service.details;
  if (inspect) {
    ...
    if (inspect.store) lines.push(`      Store：${inspect.store}`);
  ...
  lines.push(`    Capabilities：${service.capabilities.join(", ") || "无"}`,
    ...
    `    Stores：${stores.map(store => `${store.id} (${store.kind})`).join(", ") || "未声明"}`,
    "    Workloads：");
  ...
  for (const dependency of dependencies) {
    lines.push(`      ${dependency.id}: ${dependency.service}/${dependency.capability}/${dependency.store}`);
  }
