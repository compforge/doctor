...
Pydump 会通过 GDB/ptrace 把有界 C Agent 加载到目标解释器，由执行容器内的 Collector
...
探测已有 doctor debug container
  ...
  └─ 不可用：检查目标 container 是否已具备完整 attach 前置
                 ↓
              按目标平台和 CPython minor 从 Doctor Toolkit 临时上传 Collector 与 Agent
  ↓
展示影响并取得确认
  ↓
...
- Python 3；
- GDB，且 Doctor 自建临时 Python 进程的 inferior function call 验收通过；
- Pydump Collector 和匹配目标 CPython minor、架构与最低 glibc 兼容版本的 Agent（缺少时可从 Toolkit 上传）；
...
`/tmp/doctor-pydump` 再 attach。Pydump 不依赖 GDB Python scripting。
...
GDB 可执行文件存在不代表能在当前 Target kernel 上正确保存和恢复 inferior 的寄存器状态；
容器声明 `SYS_PTRACE` 同样不代表运行态 attach 一定可行。Doctor 在上传和确认前分别验证
Python、GDB 临时 inferior call、临时目录与实际 ptrace
条件，任一缺失都停止。
...
`doctor mem` 所需 Python、GDB 和 ptrace 属于诊断目标；`doctor mema` 所需 Go analyzer 或本地
