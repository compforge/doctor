...
7. HTML 和 Markdown 分别展示 Pod、Toolchain、应用依赖与配置对照；Toolchain 明确标为声明，依赖与 runtime version 明确标为本次现场观测。
8. 用户选择只采集 Pod 运行态时，报告按 partial 正常交付；完全没有形成可用证据或交付失败时才回退失败 Evidence Bundle。Kubernetes 原始响应不落盘，报告只保存归并后的事实。
